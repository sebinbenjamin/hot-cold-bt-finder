...
Verification is manual and needs real hardware: Chrome on Android with Bluetooth on and the **Nearby devices** permission granted (Android 12+; `ACCESS_FINE_LOCATION` plus the system Location toggle on Android 10–11). `requestLEScan` **and** `watchAdvertisements` are both behind `chrome://flags/#enable-experimental-web-platform-features`, on every platform including Android — `requestDevice` itself is not, but the picker path yields no RSSI without `watchAdvertisements`, so the flag is required either way.

**Scanning has only ever worked for us on Android.** Chromium implements advertisement delivery on every platform (Windows via `BluetoothAdapterWinrt`, macOS via CoreBluetooth, Linux/ChromeOS via BlueZ/Floss), so there is no platform where it fails by construction. But on our Windows machine the scan starts and no advertisements ever arrive, and Google's own [scanning sample](https://googlechrome.github.io/samples/web-bluetooth/scan.html) fails identically there — so it isn't this app. The WebBluetoothCG [status table](https://github.com/WebBluetoothCG/web-bluetooth/blob/main/implementation-status.md) also lists scanning as Android/macOS only. `scanningActuallyWorks()` gates the user-facing copy conservatively on that experience, not on a known platform gap.

Cause of the Windows failure is **unresolved** — see `docs/research/web-bluetooth-scanning-support.md`. Do not write a new explanation into this file without first running the `chrome://bluetooth-internals`, `chrome://device-log`, and picker checks listed there. (An earlier version of this document blamed "Windows 8 APIs with no scanning support"; that came from a 2018 post predating the WinRT migration and is wrong.)
...
- `startScan()` → `navigator.bluetooth.requestLEScan()` with a *global* `advertisementreceived` listener on `navigator.bluetooth` (`onAdvert`). Flag-gated; guarded by two watchdogs. `SCAN_PERMISSION_TIMEOUT` (20s) races the `requestLEScan()` call itself — note that with `acceptAllAdvertisements` the promise is *designed* not to settle until the user answers the permission prompt, so this timer fires on slow prompt-answering too and can't distinguish that from a real hang. A second watchdog at 7s after the scan starts reports missing permissions when the scan runs but no adverts arrive.
